...
export const CodeGenerationApi = {
  ...
  getCodeGenerationList: async (params: PageParams): PageRes<CodeGenerationVO> => {
    return await request.get({ url: '/platform-api/operations/code-generation/list', params })
  },

  /** 新增模組 */
  createCodeGeneration: async (data: CodeGenerationVO) => {
    return await request.post({ url: '/platform-api/operations/code-generation/create', data })
  },

  /** 修改模組 */
  updateCodeGeneration: async (data: CodeGenerationVO) => {
    return await request.put({ url: '/platform-api/operations/code-generation/update', data })
  },

  /** 刪除模組 */
  deleteCodeGeneration: async (id: string) => {
    return await request.delete({ url: `/platform-api/operations/code-generation/delete/${id}` })
  },

  /** 封鎖模組 */
  blockCodeGeneration: async (id: string) => {
    return await request.put({ url: `/platform-api/operations/code-generation/block/${id}` })
  },

  /** 解封模組 */
  unblockCodeGeneration: async (id: string) => {
    return await request.put({ url: `/platform-api/operations/code-generation/unblock/${id}` })
  },

  /** 生成數據表的代碼返回前端進行預覽 */
  previewEntityCode: async (data: CodeGenerateEntityParamsVO): ApiResponse<{ treeData: TreeData[] }> => {
    return await request.post({ url: `/platform-api/operations/code-generation/preview-entity-code`, data })
  },

  /** 插入實體代碼 */
  insertEntityCode: async (data: CodeGenerateEntityParamsVO) => {
    return await request.post({ url: `/platform-api/operations/code-generation/insert-entity-code`, data })
  },

  /** 獲取 Entity 中的所有自訂欄位 */
  getEntityCustomFields: async (params: { splitName: string }) => {
    return await request.get({ url: '/platform-api/operations/code-generation/get-entity-custom-fields', params })
  },

  /** 預覽後端代碼 */
  previewBackendCode: async (data: CodeGenerateBackendParamsVO): ApiResponse<{ treeData: TreeData[] }> => {
    return await request.post({ url: '/platform-api/operations/code-generation/preview-backend-code', data })
  },

  /** 插入後端代碼 */
  insertBackendCode: async (data: CodeGenerateBackendParamsVO) => {
    return await request.post({ url: '/platform-api/operations/code-generation/insert-backend-code', data })
  },

  // 獲取指定的Entity 中 所有欄位
  getEntityAllFields: async (params: { splitName: string }): ApiResponse<EntityAllFieldsVO> => {
    return await request.get({ url: '/platform-api/operations/code-generation/get-entity-all-fields', params })
  },
