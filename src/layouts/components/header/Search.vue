...
watchEffect(() => {
  // 沒有打開彈出視窗或沒有搜索結果時，不操作
  if (!showModal.value || !options.value.length)
    return

  // 設置鍵盤操作鎖，設置後不會被動觸發mouseover
  setKeyboardTrue()
  if (arrowup.value)
    handleArrowup()
...
function handleScroll(currentIndex: number) {
  // 保持6個選項在可視區域內,6個後開始滾動
  const keepIndex = 5
  // 單個元素的高度，包括了元素的gap和容器的padding
  const elHeight = 70
...
<template>
  ...
  <n-modal
    v-model:show="showModal" class="w-560px fixed top-60px inset-x-0 max-w-full" size="small" preset="card"
    :segmented="{
      content: true,
      footer: true,
  ...
  >
    <template #header>
      <n-input
        v-model:value="searchValue" :placeholder="$t('app.searchPlaceholder')" clearable size="large"
    ...
    <n-scrollbar ref="scrollbarRef" class="h-450px">
      <ul v-if="options.length" class="flex flex-col gap-8px p-8px p-r-3">
        <n-el
          v-for="(option, index) in options" :key="option.value" tag="li" role="option"
